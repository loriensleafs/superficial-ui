...
function createIconTyping(file) {
  ...
  const contents = `export { Icon } from '@superficial-ui/components/Icon';`;
...
function createIndexTyping(files) {
  const contents = `
import { Icon } from '@superficial-ui/components/Icon';

type IconComponent = typeof Icon;

${files
  .map(file => `export const ${normalizeFileName(file)}: IconComponent;`)
  .join('\n')}
`;
